...
#'
#' @export
group_similarity <- function(dissimilarity, weight, groups) {
  ...
  group_weights = as.vector(t(weight) %*% group_membership_matrix) # weight of parties
  emission_matrix = diag(weight) %*% group_membership_matrix %*% diag(1/group_weights)
